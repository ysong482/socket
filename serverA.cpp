...
string Graph::searchMatrix(char vertex) {
	...
	for (int i = 0;i < numVerts;i++)//search common neighbor
	{
        for (int j=0;j< numVerts;j++){
		if (adjcentMat[i][vertexInt] ==1 && adjcentMat[i][j] == 1 && j!=vertexInt &&i!=vertexInt && adjcentMat[j][vertexInt]==0) {
            neiborcount[j]++;
			}
        }
	}
...
void sendAnswer(Graph country,unordered_map<string, int> user_backend_mapping,string UID,int socket,sockaddr_in udpaddr,string Cname){
	...
	cout<<p<<endl;
	cout<<resValue[0]<<" "<<resValue[1]<< endl;
	...
	cout<<"Here are the resultes"<<RUID<<endl;
    if(resValue[0]== "0"){//in this situation have no common neighbor
        AWS ="is already connected to all other users, no new recommendation";
    }else{//edit the answer we get
        AWS ="recommend new firend "+RUID+" to user "+UID+" (common neighbors = "+resValue[0]+" )\n";
    }
...
int main(){
	...
	while (getline(infile, str2))//read the data line by line,code from sample
	{
    ...
    //create the Server A socket,code from beej
    int serverA_socket;
    serverA_socket = socket(AF_INET, SOCK_DGRAM,0);

    //define the Server A address,code from beej
	...
	//boot up message
	cout<<"The server A is up and running using UDP on port 30112"<<endl;

    //bind the socket to our specified IP and port,code from beej
    ...
    while(1)
    {
        ...
        recvfrom(serverA_socket,&recvBuffer,sizeof(recvBuffer),0, (struct sockaddr*)&udp_address, &addr_size);
        ...
        if(received_req == command){
            sendto(serverA_socket, (const char *)countryList, strlen(countryList),0,(const struct sockaddr*)&udp_address, sizeof(udp_address));
			...
			cout<<"The server A has received request for finding possible friends of User "<<resA[1]<<" in "<< resA[0]<<endl;
            ...
            if(matrixID== 0){//country name has matrix country1
                sendAnswer(country1,user_backend_mapping1,resA[1],serverA_socket,udp_address,resA[0]);
				continue;
            }else if(matrixID==1){
                sendAnswer(country2,user_backend_mapping2,resA[1],serverA_socket,udp_address,resA[0]);
				continue;
            }else if(matrixID==2){
                sendAnswer(country3,user_backend_mapping3,resA[1],serverA_socket,udp_address,resA[0]);
				continue;
            }else if(matrixID==3){
                sendAnswer(country4,user_backend_mapping4,resA[1],serverA_socket,udp_address,resA[0]);
				continue;
            }else if(matrixID==4){
                sendAnswer(country5,user_backend_mapping5,resA[1],serverA_socket,udp_address,resA[0]);
				continue;
            }else if(matrixID==5){
                sendAnswer(country6,user_backend_mapping6,resA[1],serverA_socket,udp_address,resA[0]);
				continue;
            }else if(matrixID==6){
                sendAnswer(country7,user_backend_mapping7,resA[1],serverA_socket,udp_address,resA[0]);
				continue;
            }else if(matrixID==7){
                sendAnswer(country8,user_backend_mapping8,resA[1],serverA_socket,udp_address,resA[0]);
				continue;
            }else if(matrixID==8){
                sendAnswer(country9,user_backend_mapping9,resA[1],serverA_socket,udp_address,resA[0]);
				continue;
            }else if(matrixID==9){
                sendAnswer(country10,user_backend_mapping10,resA[1],serverA_socket,udp_address,resA[0]);
				continue;
            }else{
				continue;
            }
